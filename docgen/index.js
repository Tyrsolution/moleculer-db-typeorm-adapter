...
function hasTag(item, tagName) {
	if (item.tags) return item.tags.find((tag) => tag.title == tagName) != null;
...
function resolveParams(item) {
	if (item.params) return item.params.map((p) => resolveParam(p));
...
function resolveParam(p) {
	...
	if (defaultValue === undefined) {
		if (!p.optional && !p.nullable) defaultValue = '**required**';
		else defaultValue = '-';
	} else defaultValue = '`' + JSON.stringify(defaultValue, null, 2) + '`';
...
function transformReadme({ doc, template }) {
	...
	const transforms = {
		...
		SETTINGS(doc) {
			...
			let blocks = doc
				.filter((item) => item.memberof && item.memberof.startsWith(prefix))
				.map((item) => {
					let defaultValue =
						item.meta && item.meta.code ? item.meta.code.value : undefined;

					if (defaultValue != null) {
						if (!item.optional && !item.nullable) defaultValue = '**required**';
			...
			blocks.forEach((item) =>
				console.log(' - ' + chalk.white.bold(item.name) + ': ' + trunc(item.description)),
			);
			console.log('');
		...
		PROPERTIES(doc) {
			console.log(chalk.yellow.bold('Generating properties...'));
			let blocks = doc
				.filter((item) => hasTag(item, 'properties'))
				.map((item) => {
					return {
						name: item.name,
						description: item.description,
		...
		ACTIONS(doc) {
			...
			let blocks = doc
				.filter((item) => hasTag(item, 'actions'))
				.map((item) => {
					return {
						name: item.name,
						description: item.description,
		...
		METHODS(doc) {
			...
			let blocks = doc
				.filter((item) => hasTag(item, 'methods'))
				.map((item) => {
					return {
						name: item.name,
						description: item.description,
		...
		CONNECTIONMANAGER(doc) {
			console.log(chalk.yellow.bold('Generating connectionmanager...'));
			let blocks = doc
				.filter((item) => hasTag(item, 'connectionmanager'))
				.map((item) => {
					return {
						name: item.name,
						description: item.description,
...
function render(template, name, data) {
	let reTemplate = new RegExp(
		`\\<\\!--\\s*?AUTO-CONTENT-TEMPLATE:${name}(?:\\r?\\n)+((?:.|\\s)*?)--\\>`,
		'g',
	);
	...
	if (tmpl && tmpl.length > 1) {
		...
		let reContent = new RegExp(
			`(\\<\\!--\\s*?AUTO-CONTENT-START:${name}\\s*?--\\>\\s?)((?:.|\\s)*?)(<!--\\s*?AUTO-CONTENT-END:${name}\\s*?--\\>)`,
			'g',
		);
